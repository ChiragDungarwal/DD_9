...
function initSlideshow() {
  ...
  function goToSlide(index) {
    ...
    // Unlock transition after CSS transition ends (0.6s)
    setTimeout(() => {
      isTransitioning = false;
  ...
  // Pause on hover (over the whole carousel wrapper)
...
function expandToProduct() {
  ...
  // Wait for DOM to be fully loaded and dropdowns to be initialized
  const attemptExpand = (attempts = 0) => {
    if (attempts > 20) return; // Stop after 2 seconds
    
    const categories = document.querySelectorAll('.product-category');
    if (categories.length === 0) {
      setTimeout(() => attemptExpand(attempts + 1), 100);
      return;
    }
    ...
    for (const cat of categories) {
      ...
      if (expandData.category === 'bch' && catText.includes('BCH Products')) {
        ...
        if (expandData.section) {
          ...
          setTimeout(() => {
            scrollToProductByTitle(expandData.productTitle);
          }, 200);
        }
        expanded = true;
        break;
        ...
        setTimeout(() => {
          ...
          for (const nested of nestedSubcats) {
            ...
            if (nestedText === subsectionName) {
              ...
              setTimeout(() => {
                scrollToProductByTitle(productTitle);
              }, 200);
              break;
            }
          }
...
// Scroll to specific product card by title (global search)
function scrollToProductByTitle(productTitle) {
  if (!productTitle) return false;
  
  // Try multiple selectors to find product cards
  const selectors = ['.product-card', '.product-item', '.product', '[data-product]'];
  let productCards = [];
  ...
  for (const card of productCards) {
    const titleElem = card.querySelector('h3, .product-title, [data-title]');
...
// NEW FUNCTION: Direct product scrolling without relying on sessionStorage
function directScrollToProduct(productTitle, category, section, subsection) {
  // Store the search intent
  sessionStorage.setItem('expandProduct', JSON.stringify({
  ...
  // If we're already on products page, trigger expansion immediately
  if (window.location.pathname.includes('products.html') || window.location.pathname.endsWith('products')) {
    expandToProduct();
  } else {
  ...
  for (const item of searchIndex) {
    ...
    if (score > 0) {
      results.push({
        ...
        anchor: item.anchor // Store anchor for direct access
      });
    }
  }
...
// NEW FUNCTION: Handle search result click
function handleSearchResultClick(result) {
  if (result.page.includes('products.html') || result.page === 'products.html') {
    // Store the product info for expansion
    sessionStorage.setItem('expandProduct', JSON.stringify({
    ...
    if (window._expandTimeout) {
      clearTimeout(window._expandTimeout);
    }
    
    // Navigate to products page
    window.location.href = result.page;
  } else if (result.page === 'index.html') {
    window.location.href = 'index.html';
  } else if (result.page === 'contact.html') {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Check for stored search intent on products page
  if (window.location.pathname.includes('products.html') || window.location.pathname.endsWith('products')) {
  ...
  // Set up search input listener (if search box exists)
...
// Display search results (Updated to use handleSearchResultClick)
function displaySearchResults(results, container) {
  ...
  const html = results.map(result => `
    <div class="search-result-item" data-title="${result.title}" data-category="${result.category || ''}" data-section="${result.section || ''}" data-subsection="${result.subsection || ''}" data-page="${result.page}">
      <div class="search-result-title">${result.title}</div>
      <div class="search-result-path">${result.page.replace('.html', '')} ${result.section ? `› ${result.section}` : ''} ${result.subsection ? `› ${result.subsection}` : ''}</div>
      <div class="search-result-snippet">${result.matches.join(' • ')}</div>
    </div>
  `).join('');
  ...
  items.forEach(item => {
    item.addEventListener('click', function() {
      const result = {
        ...
        category: this.dataset.category || null,
        section: this.dataset.section || null,
        subsection: this.dataset.subsection || null,
